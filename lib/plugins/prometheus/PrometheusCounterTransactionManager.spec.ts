...
describe("PrometheusCounterTransactionManager", () => {
  ...
  describe("appMetrics is defined", () => {
    ...
    it('returns counter metrics', async () => {
      const counterManager = new PrometheusCounterTransactionManager('myMetric', 'this is my first metric', app.metrics)
      ...
      const counterManager2 = new PrometheusCounterTransactionManager('myMetric', 'this is my second metric', app.metrics)
      counterManager2.start('myTransaction', 'myKey4')
      counterManager2.stop('myKey4', false)

      const response2 = await sendGet(buildClient('http://127.0.0.1:9080'), '/metrics', TEST_OPTIONS)
